...
use bex::{find_factors, Base, bdd::BddBase, int::{X8,X16}, swap::SwapSolver, reg::Reg};
...
pub fn main() {
  ...
  for a in std::env::args() {
    if get_threads { num_threads = a.parse().expect("bad -t parameter"); get_threads=false; }
      ...
      "swap" => use_swap = true,
      _ => { /* ignore for now */} }}}
  println!("[bex {BEX_VERSION} -O{BEX_OPT_LEVEL}] factor-p4 ({})",
  if use_swap { "swap solver" } else { "sub solver" });
  if num_threads != 0 { println!("note: t parameter is ignored"); }
  if use_swap { find_factors!(SwapSolver, X8, X16, K as usize, expected); }
  else { find_factors!(BddBase, X8, X16, K as usize, expected); }}
