...
pub fn refine<P:Progress>(dest: &mut B, src:&ASTBase, end:nid::NID, pr:P)->nid::NID {
  let mut topnid = convert_nid(end);
  println!("INITIAL TOPNID: {:?}", topnid);
...
pub fn convert_nid(n:ast::NID)->nid::NID {
  if nid::is_const(n) { n }
  else if nid::is_var(n) { nid::nvr(nid::var(n)) }
  else if nid::var(n) == nid::NOVAR { nid::nv(nid::idx(n)) }
  else { todo!("convert_nid({:?})", n) }}

fn refine_one(dst: &mut B, src:&ASTBase, n:nid::NID)->nid::NID {
  println!("refine_one({:?})", n);
  if nid::is_const(n) { n }
  else if nid::is_var(n) { nid::nvr(nid::var(n)) }
  else if nid::var(n) == nid::NOVAR {
    let otv = nid::idx(n);
    let op = src.get_op(n);
    let v = |x0:ast::NID|->nid::NID { convert_nid(x0) };
    println!("op: {:?}", op);
    ...
    println!("sub(otv:{:?}, new:{:?}, old:{:?})", otv, newdef, n);
    dst.sub(otv, newdef, n) }
  else { todo!("refine_one({:?})", n) }}
