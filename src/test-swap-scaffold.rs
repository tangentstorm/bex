...
#[test] fn test_scaffold_diagram0() {
    ...
    assert_eq!(1, xs.vhls.len(), "expecting only Vhl_O at this point");
    let mut node = |v,hi,lo|->NID { xs.add_ref(Vhl{v, hi, lo}, 0, 0) };
    let xo = crate::nid::O;
    const SLOTS:usize = 9;

    // leave some space for the numbered nodes in the diagrams by creating fake nodes:
    // (can use nid::O directly, but we create fake nodes to leave slots for numbered nodes so add_ref would overwrite the empty slot)
  ...
  let n2 = xs.add_ref(Vhl{ v:vu, hi:a2, lo:a3 }, 0, 0);
  ...
  let n1 = xs.add_ref(Vhl{ v:vd, hi:a0, lo:a1 }, 0, 0);
  ...
  let z0 = xs.add_ref(Vhl{ v:vz, hi:n1, lo:n2 }, 0, 0);
  assert_eq!(z0.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for z0.");
    ...
    while i <= SLOTS { xs.del_node(NID::ixn(i as i64 as usize)); i+=1; }
  ...
  assert_eq!(xs.nids_on_row(vu), HashSet::from_iter(vec![n2]), "row u didn't match expected values!");
  assert_eq!(xs.nids_on_row(vd), HashSet::from_iter(vec![n1]), "row d didn't match expected values!");
...
#[test] fn test_scaffold_diagram1() {
  ...
  let (old_xid, old_vhl) = (n2, xs.get(n2).unwrap()); xs.del_node(n2);
  let n2 = xs.add_ref(Vhl{ v:vu, hi:a1, lo:a2 }, 0, 0);
  assert_eq!(n2.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n2.");
  let (old_xid, old_vhl) = (n1, xs.get(n1).unwrap()); xs.del_node(n1);
  let n1 = xs.add_ref(Vhl{ v:vd, hi:a0, lo:n2 }, 0, 0);
  assert_eq!(n1.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n1.");
  let (old_xid, old_vhl) = (z0, xs.get(z0).unwrap()); xs.del_node(z0);
  let z0 = xs.add_ref(Vhl{ v:vz, hi:old_vhl.hi, lo:old_vhl.lo }, 0, 0);
  assert_eq!(z0.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for z0.");
  ...
  assert_eq!(xs.nids_on_row(vu), HashSet::from_iter(vec![]), "row u didn't match expected values!");
  assert_eq!(xs.nids_on_row(vd), HashSet::from_iter(vec![]), "row d didn't match expected values!");
...
#[test] fn test_scaffold_diagram2() {
  ...
  let (old_xid, old_vhl) = (n2, xs.get(n2).unwrap()); xs.del_node(n2);
  let n2 = xs.add_ref(Vhl{ v:vu, hi:a0, lo:a1 }, 0, 0);
  assert_eq!(n2.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n2.");
  let (old_xid, old_vhl) = (n1, xs.get(n1).unwrap()); xs.del_node(n1);
  let n1 = xs.add_ref(Vhl{ v:vd, hi:n2, lo:a2 }, 0, 0);
  assert_eq!(n1.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n1.");
  let (old_xid, old_vhl) = (z0, xs.get(z0).unwrap()); xs.del_node(z0);
  let z0 = xs.add_ref(Vhl{ v:vz, hi:n1, lo:old_vhl.lo }, 0, 0);
  assert_eq!(z0.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for z0.");
  ...
  assert_eq!(xs.nids_on_row(vu), HashSet::from_iter(vec![n1]), "row u didn't match expected values!");
  assert_eq!(xs.nids_on_row(vd), HashSet::from_iter(vec![n2, n3]), "row d didn't match expected values!");
...
#[test] fn test_scaffold_diagram3() {
  ...
  let z1 = xs.add_ref(Vhl{ v:vz, hi:n2, lo:old_vhl.lo }, 0, 0);
  ...
  assert_eq!(xs.nids_on_row(vu), HashSet::from_iter(vec![n2, n1]), "row u didn't match expected values!");
  assert_eq!(xs.nids_on_row(vd), HashSet::from_iter(vec![n3, n4]), "row d didn't match expected values!");
...
#[test] fn test_scaffold_diagram4() {
  ...
  let n3 = xs.add_ref(Vhl{ v:vu, hi:a2, lo:a3 }, 0, 0);
  assert_eq!(n3.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n3.");
  let (old_xid, old_vhl) = (n2, xs.get(n2).unwrap()); xs.del_node(n2);
  let n2 = xs.add_ref(Vhl{ v:vu, hi:a0, lo:a1 }, 0, 0);
  assert_eq!(n2.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n2.");
  let (old_xid, old_vhl) = (n1, xs.get(n1).unwrap()); xs.del_node(n1);
  let n1 = xs.add_ref(Vhl{ v:vd, hi:n2, lo:n3 }, 0, 0);
  assert_eq!(n1.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for n1.");
  let (old_xid, old_vhl) = (z0, xs.get(z0).unwrap()); xs.del_node(z0);
  let z0 = xs.add_ref(Vhl{ v:vz, hi:n1, lo:old_vhl.lo }, 0, 0);
  assert_eq!(z0.raw(), old_xid.raw(), "scaffold failed to reuse empty slot for z0.");
